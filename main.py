...
print("Loading model")
# Load with SDPA instead of FlashAttention
model = Qwen3TTSModel.from_pretrained(
    "Qwen/Qwen3-TTS-12Hz-0.6B-Base",  # Use 0.6B for 8GB VRAM
    device_map="cuda:0",
    dtype=torch.bfloat16,
    attn_implementation="sdpa",  # ← Just change this one line
)
...
conversation = scriptMaker.generateScript(
...
# added the topic to speaker_a's lines
...
# dividing lines based on batching count
...
for i, batch in enumerate(batched_speaker_a_lines):
    ...
    # 2. Move to CPU and convert to numpy to free VRAM
    ...
    # 3. Explicitly clear unused VRAM
...
# Save final output
sf.write("full_episode.wav", combined_dialog, sr)
