document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadHistory() {
        try {
            ...
            projects.forEach(p => {
                ...
                const url = p.article_url || '';

                card.innerHTML = `
                    <div class="card-info">
                        <div class="card-title">${escapeHtml(p.article_title || 'Untitled')}</div>
                        <div class="card-meta">
                            <span>${date}</span>
                            ${url ? `<a href="${escapeHtml(url)}" target="_blank" rel="noopener">Open article &nearr;</a>` : ''}
                        </div>
                    </div>
                    <div class="card-actions">
                        <button class="btn small secondary play-btn" title="Play podcast">&#9654;</button>
                        <button class="btn small secondary view-script-btn" title="View script">Script</button>
                        <button class="btn small icon delete-project-btn" title="Delete" style="color:var(--text-secondary); padding:8px;">&times;</button>
                    </div>
                `;

                card.querySelector('.play-btn').addEventListener('click', () => playHistoryPodcast(p));
                card.querySelector('.view-script-btn').addEventListener('click', () => viewHistoryScript(p.id));
                card.querySelector('.delete-project-btn').addEventListener('click', () => deleteHistoryProject(p.id, card));
    ...
    function playHistoryPodcast(project) {
        const audio = document.getElementById('final-audio');
        audio.querySelector('source').src = `/static/podcasts/${project.podcast_file}`;
        audio.load();
        audio.play();
    ...
    async function viewHistoryScript(projectId) {
        try {
            const resp = await fetch(`/api/projects/${projectId}`);
            const project = await resp.json();
            if (!project || !project.dialog) return;

            const overlay = document.createElement('div');
            overlay.className = 'script-modal-overlay';

            let html = `<div class="script-modal"><button class="modal-close">&times;</button><h3>${escapeHtml(project.article_title || 'Script')}</h3>`;

            const entries = project.dialog.script || [];
            entries.forEach(entry => {
                const cls = entry.speaker === 'Host_B' ? 'speaker-b' : 'speaker-a';
                const label = entry.speaker === 'Host_B' ? 'Speaker B' : 'Speaker A';
                html += `<div class="dialog-entry ${cls}"><div class="speaker-label">${label}</div><div>${escapeHtml(entry.text)}</div></div>`;
            });
    ...
    async function deleteHistoryProject(projectId, card) {
        try {
